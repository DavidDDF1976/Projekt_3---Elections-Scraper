...
def get_voters_envelopes_valid_votes(url_municipality):
  ...
  get_voters_envelopes_valid_votes_list = [
      symbol.replace(r"\xa0", " ")
      for symbol in get_voters_envelopes_valid_votes_list
  ]
  ...
  try:
    while True:
      parser = argparse.ArgumentParser(
          description="Vložte odkaz na vybraný okres a název csv souboru "
          "včetně přípony .csv")
      parser.add_argument(
          'district_url',
          help="Vložte odkaz na vybraný okres s uvozovkami",
          type=str)
      parser.add_argument('title_csv',
                          help="Vložte jméno csv souboru včetně přípony .csv",
                          type=str)
      ...
      if args.district_url[0:52] == "https://volby.cz/pls/ps2017nss/ps32?xjazyk=CZ&xkraj=" \
        ...
        print(
            "Špatně zadaný odkaz nebo název csv souboru, zkuste vložit znovu"
        )
        exit()
    ...
    df = pd.DataFrame(
        results,
        columns=(["location", "registered", "envelopes", "valid"] + parties))
...
if __name__ == "__main__":
  ...
  sys.argv.append("results_prostejov.csv")
  main()
  print("DONE!")
